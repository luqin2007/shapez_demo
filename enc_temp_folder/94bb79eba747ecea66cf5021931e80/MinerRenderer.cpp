                                             ...
                                             const float x0, const float y0, const float x1, const float y1)
{
	...
	case Side::up:
		return {
			{x0 + 0.3f * cell_size, y0 - (pp + 0.2f) * cell_size},
			{x1 - 0.3f * cell_size, y0 - (pp - 0.2f) * cell_size}
		};
	case Side::down:
		return {
			{x0 + 0.3f * cell_size, y1 + (pp - 0.2f) * cell_size},
			{x1 - 0.3f * cell_size, y1 + (pp + 0.2f) * cell_size}
		};
	case Side::left:
		return {
			{x0 - (pp - 0.2f) * cell_size, y0 + 0.3f * cell_size},
			{x0 - (pp + 0.2f) * cell_size, y1 - 0.3f * cell_size}
		};
	case Side::right:
		return {
			{x1 + (pp - 0.2f) * cell_size, y0 + 0.3f * cell_size},
			{x1 + (pp + 0.2f) * cell_size, y1 - 0.3f * cell_size}
		};
	}
	return {};
}
