                                  ...
                                  GameRenderer& renderer, const GameMap& map) const
{
	...
	if (const auto res = map.get_resource(row, col); res != ResourceType::none)
	{
		...
		if (draw_res)
		{
			...
			renderer.tex_drawer.push(p0.x, p0.y, p1.x, p1.y, renderer.atlas, resource_icon(res), context.direction);
		}
	}
                                             ...
                                             const float x0, const float y0, const float x1, const float y1)
{
	...
	case Side::down:
		return {
			{x0 + 0.25f * cell_size, y1 + (pp - 0.2f) * cell_size},
			{x1 - 0.25f * cell_size, y1 + (pp + 0.3f) * cell_size}
		};
	case Side::left:
		return {
