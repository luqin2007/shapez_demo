                                    ...
                                    GameRenderer& renderer, const GameMap& map) const
{
	...
	renderer.tex_drawer.push(x0 - EXT_SIZE, y0 - EXT_SIZE, x1 + EXT_SIZE, y1 + EXT_SIZE, renderer.atlas, "stacker.png",
	                         context.direction);
}

void StackerRenderer::draw_holding(const float x, const float y, const Side direction, GameRenderer& renderer,
                                   const GameMap& map) const
{
	renderer.tex_drawer.push(x - EXT_SIZE, y - EXT_SIZE, x + map.cell_size * 2 + EXT_SIZE, y + map.cell_size + EXT_SIZE,
	                         renderer.atlas, "stacker_blue.png", direction);
}

void StackerRenderer::draw_icon(const float x, const float y, float size, GameRenderer& renderer) const
{
